...
- Master-resume uploads enforce a 10 MiB ceiling and validate supported
  ...
  Ad hoc Auto-fill uploads use the same boundary, remove staged bytes when
  attachment fails, and update the future master pointer only after successful
  attachment. Malware scanning and retention/deletion controls are not implemented.
